...
export class MapLayers {
  ...
  removeCurrentLayer() {
    try {
      ...
      if (this.currentLayer) {
        ...
        // Handle old structure or remaining layers by source
          ...
          const layers = this.map
            .getStyle()
            .layers.filter(layer => layer.source === this.currentLayer.sourceId);
      ...
      // Final verification - list all remaining layers (unused)
      // const _remainingLayers = this.map.getStyle().layers.map(l => l.id);
    } catch (error) {
  ...
  isPreviewAsset(asset) {
    ...
    // Check by media type
    if (asset.type) {
      const previewTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
  ...
  isVisualAsset(asset) {
    ...
    return visualTypes.some(
      type =>
        asset.type.toLowerCase().includes(type) ||
        asset.href?.toLowerCase().includes(type.split('/')[1])
    );
  }
  ...
  reapplyThumbnails() {
    ...
    thumbnails.forEach((_sourceId, _itemId) => {
  ...
  async displayItemOnMap(item, preferredAssetKey = null) {
    try {
      ...
      if (item.assets?.rendered_preview?.href.includes('planetarycomputer')) {
  ...
  async addImageWithValidation(url, bbox, item) {
    ...
    try {
      ...
      const coordinates = this.calculateOptimalCoordinates(imageData, bbox);
   ...
   */
  calculateOptimalCoordinates(imageData, bbox) {
    ...
    if (imgAspectRatio > bboxAspectRatio) {
      ...
      // const _heightDiff = bboxHeight - adjustedHeight; // unused
      const midY = (bbox[1] + bbox[3]) / 2;

      adjustedBbox = [bbox[0], midY - adjustedHeight / 2, bbox[2], midY + adjustedHeight / 2];
    } else {
      // Image is taller than bbox
      const adjustedWidth = bboxHeight * imgAspectRatio;
      // const _widthDiff = bboxWidth - adjustedWidth; // unused
      const midX = (bbox[0] + bbox[2]) / 2;

      adjustedBbox = [midX - adjustedWidth / 2, bbox[1], midX + adjustedWidth / 2, bbox[3]];
    }
  ...
  useResultsThumbnail(item) {
    try {
      ...
      const resultItem = this.findResultItemByMultipleStrategies(item.id);
  ...
  findResultItemByMultipleStrategies(itemId) {
    ...
    for (const element of elements) {
      if (element.textContent?.includes(itemId)) {
