...
import { getMapManager, MapManager, setMapManager } from './components/map/MapManager.js';
...
// import { searchHistoryUI } from './components/ui/SearchHistoryUI.js'; // Temporarily unused
...
// import { searchHistoryManager } from './utils/SearchHistoryManager.js'; // Temporarily unused
...
async function initAppForBrowserMode() {
  try {
    ...
    const collectionManager = new CollectionManagerEnhanced(
      apiClient,
      notificationService,
      catalogSelector,
      CONFIG
    ...
    const filterContainer =
      document.getElementById('smart-filters-container') ||
    ...
    // Initialize collection selector integration (currently unused)
    // const _collectionSelectorIntegration = new CollectionSelectorIntegration(
    //   collectionManager,
    //   apiClient,
    //   notificationService,
    //   CONFIG
    ...
    if (collectionManager?.allCollections && collectionManager.allCollections.length > 0) {
  ...
  try {
    ...
    catalogBrowser.setItemSelectHandler(item => {
      ...
      notificationService.showNotification(
        `Selected item: ${item.properties?.title || item.id}`,
        'info'
      );
    });
    ...
    catalogBrowser.setCollectionSelectHandler(collection => {
      ...
      if (collectionManager) {
        ...
        notificationService.showNotification(
          `Selected collection: ${collection.title || collection.id}`,
          'info'
        );
      }
    });
    ...
    const { RasterVisualizationManager } = await import(
      './components/visualization/RasterVisualizationManager.js'
    );
        ...
        action: () => {
          ...
          if (dateRange.start && dateRange.end) {
            ...
            if (window.stacExplorer?.inlineDropdownManager) {
              ...
              window.stacExplorer.inlineDropdownManager.updateSearchSummary(
                'date',
...
function setupLocationInputs(inlineDropdownManager) {
  ...
  locationInputs.forEach(input => {
    ...
    input.addEventListener('input', async e => {
      ...
      resultsContainer.innerHTML =
        '<div style="padding: 12px; color: #666; text-align: center;">Searching...</div>';

      // Import and use the geocoding service
      import('./utils/GeocodingService.js')
        .then(module => {
          ...
          defaultGeocodingService.searchLocations(query, (results, error) => {
            if (error) {
              resultsContainer.style.display = 'block';
              resultsContainer.innerHTML =
                '<div style="padding: 8px; color: #666;">Error searching locations</div>';
              return;
            }
            ...
            if (results.length === 0) {
              resultsContainer.style.display = 'block';
              resultsContainer.innerHTML =
                '<div style="padding: 8px; color: #666;">No locations found</div>';
              return;
            }
        ...
        .catch(_error => {
          resultsContainer.innerHTML =
            '<div style="padding: 8px; color: #999;">Location search unavailable</div>';
        });
    });
  });
  ...
  if (drawBboxBtn) {
    drawBboxBtn.addEventListener('click', e => {
      ...
      if (!mapManager || typeof mapManager.startDrawingBbox !== 'function') {
        console.error(
          '❌ Map drawing not available - MapManager or startDrawingBbox method not found'
        );
        return;
      }
      ...
      if (notificationService) {
        notificationService.showNotification(
          '🖊️ Click two points on the map to draw a bounding box',
          'info'
        );
      }
      ...
      mapManager.startDrawingBbox(bbox => {
        if (bbox && Array.isArray(bbox) && bbox.length === 4) {
          ...
          document.dispatchEvent(
            new CustomEvent('geometrySelected', {
              detail: {
                bbox: bbox,
                name: 'Custom Area',
              },
...
window.testLocationSearch = () => {
  const input =
    document.querySelector('.mini-location-input') ||
    document.querySelector('#summary-location-input');
