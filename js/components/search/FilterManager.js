...
export class FilterManager {
  ...
  setupEventListeners() {
    ...
    if (collectionSelect) {
      collectionSelect.addEventListener('change', _event => {
  ...
  showFilterModal() {
    ...
    if (existingFilterList?.innerHTML.trim()) {
  ...
  updateFiltersForCollections(collections) {
    ...
    if (relevantFilters.length > 0) {
      ...
      this.showFilterHint(
        `Found ${relevantFilters.length} applicable filter(s) for ${collectionTypes.join(', ')} data`
      );
      this.showFilterButton();
  ...
  analyzeCollectionTypes(collections) {
    ...
    collections.forEach(collection => {
      ...
      const isOptical = opticalCollections.some(
        opt => collectionId.includes(opt) || title.includes(opt) || description.includes(opt)
      );
      ...
      if (
        keywords.some(k => k.includes('optical') || k.includes('multispectral')) ||
        description.includes('optical') ||
        description.includes('multispectral') ||
        title.includes('optical') ||
        title.includes('multispectral')
      ) {
        types.add('optical');
      }
      ...
      // Radar indicators
      if (
        keywords.some(k => k.includes('radar') || k.includes('sar')) ||
        description.includes('radar') ||
        description.includes('sar') ||
        title.includes('radar') ||
        title.includes('sar') ||
        collectionId.includes('sar') ||
        collectionId.includes('sentinel-1')
      ) {
        types.add('radar');
      }
  ...
  restoreFiltersFromURL() {
    ...
    if (params.has('cc')) {
      // 'cc' is the URL key for cloudCover from UnifiedStateManager
