...
export class CommandPalette extends BaseUIComponent {
  ...
  setState(newState, _shouldRender = false) {
  ...
  updateState(newState, _shouldRender = false) {
  ...
  registerDefaultCommands() {
    ...
    this.registerCommand({
      ...
      action: () => {
        const searchBtn =
          document.getElementById('main-search-btn') ||
    ...
    this.registerCommand({
      ...
      action: () => {
        const settingsBtn =
          document.getElementById('settings-toggle') || document.querySelector('.settings-toggle');
        if (settingsBtn) {
          settingsBtn.click();
        } else {
...
Built with modern web technologies for exploring STAC-compliant data catalogs.
    ...
    if (this.backdrop) {
      this.addEventListener(this.backdrop, 'click', event => {
        ...
        if (event.target === this.backdrop) {
          ...
          // Event bubbled up from child element - do nothing
        }
      });
    }
  ...
  renderResults() {
    ...
    const groupedResults = this.options.showCategories
      ? this.groupByCategory(results)
      : new Map([['all', results]]);
    let html = '';
  ...
  updatePreview() {
    if (!this.options.enablePreview || !this.previewContainer) {
      return;
    }
      ...
      if (this.input) {
        ...
        setTimeout(() => {
          if (document.activeElement !== this.input) {
            ...
            // Input already has focus - no action needed
          }
        }, 250);
      } else {
  ...
  registerShortcut(_shortcut, _action) {
