...
export class CatalogBrowserPanel {
  ...
  renderAllCollections() {
    ...
    if (this.filteredCollections.length === 0) {
      ...
      grid.innerHTML = this.filteredCollections
        .map(collection => this.renderCollectionCard(collection))
        .join('');
    ...
    if (collectionsTitle) {
      ...
      collectionsTitle.textContent = this.searchTerm
        ? `${count} of ${total} Collections`
  ...
  /**
   * Scroll to top of collections stats section
   */
  scrollToTopStats() {
  ...
  renderCollectionCard(collection) {
    ...
    // const description = collection.description || 'No description available'; // unused
  ...
  getCollectionBadges(collection) {
    ...
    if (collection.license) {
      ...
      if (
        collection.license.toLowerCase().includes('cc') ||
        collection.license.toLowerCase().includes('creative')
      ) {
    ...
    if (collection.summaries?.gsd) {
      const gsd = Array.isArray(collection.summaries.gsd)
        ? collection.summaries.gsd[0]
        : collection.summaries.gsd;
      if (gsd <= 10) {
        badges.push({
          text: 'HIGH-RES',
          color: 'linear-gradient(135deg, #f59e0b, #d97706)',
    ...
    if (collection.extent?.temporal?.interval?.[0]) {
      ...
      const end = collection.extent.temporal.interval[0][1]
        ? new Date(collection.extent.temporal.interval[0][1])
        : new Date();
  ...
  /**
   * Setup search functionality for collections modal
   */
  setupCollectionsSearchModal(collections) {
    const searchInput = document.getElementById('collections-search');
    const clearBtn = document.getElementById('clear-collections-search');
    ...
    searchInput.addEventListener('input', e => {
      ...
      clearTimeout(searchTimeout);
      searchTimeout = setTimeout(() => {
        this.filterCollectionsModal(collections, searchTerm);
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        searchInput.value = '';
        clearBtn.style.display = 'none';
        this.filterCollectionsModal(collections, '');
        searchInput.focus();
      });
    }
  ...
  /**
   * Filter collection cards based on search term in modal
   */
  filterCollectionsModal(collections, searchTerm) {
    ...
    if (!searchTerm) {
      ...
      collectionCards.forEach(card => {
        const title =
          card.querySelector('.collection-card-title')?.textContent?.toLowerCase() || '';
        const description =
          card.querySelector('.collection-card-description')?.textContent?.toLowerCase() || '';
  ...
  createCollectionItem(collection) {
    ...
    let temporalText = '';
    if (temporalExtent?.[0] && temporalExtent[1]) {
      const start = new Date(temporalExtent[0]).getFullYear();
      const end =
        temporalExtent[1] === null ? 'present' : new Date(temporalExtent[1]).getFullYear();
  ...
  async browseCollection(collection) {
    try {
      ...
      if (Array.isArray(items)) {
        ...
        // Intentionally empty - no items found, handled by subsequent logic
      }
      ...
      this.showError(
        `Failed to load items from ${collection.title || collection.id}: ${error.message}`
      );
  ...
  displayItems(items) {
    ...
    if (!items || items.length === 0) {
      ...
      const collectionName =
        currentCollection?.data?.title || currentCollection?.data?.id || 'this collection';
  ...
  addSearchAndFilter() {
    ...
    if (!itemsSection || itemsSection.querySelector('.catalog-search-filter')) {
      return;
    }
  ...
  renderItemsFiltered(items) {
    ...
    // const searchFilter = itemsList.parentElement.querySelector('.catalog-search-filter'); // Currently unused
    itemsList.innerHTML = '';
  ...
  notifyStateChange() {
    ...
    document.dispatchEvent(
      new CustomEvent('catalogBrowserStateChanged', {
    ...
    if (window.stacExplorer?.unifiedStateManager) {
      ...
      if (
        stateManager.isRestoringFromUrl ||
        stateManager.isUpdatingFromURL ||
        stateManager.isApplyingState
      ) {
        return;
      }
  ...
  async showCatalogSelection() {
    ...
    try {
      ...
      const catalogsHTML = availableCatalogs
        .map(catalog => {
  ...
  async getAvailableCatalogs() {
    ...
    try {
      ...
      if (customCatalogData) {
        ...
        const customEndpoint = {
          ...
          collections: customCatalog.url.endsWith('/')
            ? `${customCatalog.url}collections`
            : `${customCatalog.url}/collections`,
          search: customCatalog.url.endsWith('/')
            ? `${customCatalog.url}search`
            : `${customCatalog.url}/search`,
    ...
    if (stacEndpoints) {
      ...
      const catalogPromises = entries.map(async ([key, endpoint]) => {
  ...
  createCatalogSelectionItem(catalog) {
    ...
    // const icon = this.getCatalogIcon(catalog.id); // Currently unused
      ...
      const tempApiClient = {
        ...
        async getCollections() {
          const response = await fetch(
            tempApiClient.endpoints.collections || `${tempApiClient.endpoints.root}/collections`
          );
  ...
  filterCatalogs(searchTerm) {
    ...
    catalogCards.forEach(card => {
      ...
      const description =
        card.querySelector('.catalog-description')?.textContent?.toLowerCase() || '';
    ...
    if (visibleCount === 0 && searchTerm) {
      ...
      emptyState.querySelector('p').textContent =
        `Try a different search term or clear the search to see all catalogs.`;
    } else {
