...
export class SettingsPageManager {
  ...
  hideMainAppElements() {
    ...
    const appElements = document.querySelectorAll(
      '.search-results-panel, .loading-overlay, .catalog-browser-modal'
    );
  ...
  setupEventListeners() {
    ...
    // URL input reference not needed here
  ...
  async addCustomCatalog(url) {
    const addBtn = document.getElementById('add-custom-catalog-btn');
    // Status element reference not needed here
  ...
  clearCache() {
    ...
    const keysToRemove = Object.keys(localStorage).filter(
      key => key.includes('cache') || key.includes('collections') || key.includes('stac')
    );
    ...
    notificationService.showNotification('Cache cleared successfully!', 'success');
  }
  ...
  resetSettings() {
    ...
    if (confirm('Are you sure you want to reset all settings? This cannot be undone.')) {
      ...
      // Short delay to show notification before reload
      setTimeout(() => {
