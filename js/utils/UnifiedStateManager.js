...
export class UnifiedStateManager {
  constructor(components = {}) {
    ...
    this.urlKeys = {
      ...
      browserItemId: 'itm', // selected item id (different from search itemId)
  ...
  hasSignificantUrlState(params) {
    return (
      params.has(this.urlKeys.itemId) ||
  ...
  async waitForCatalogBrowserReady() {
    ...
    // Components are ready - initialization can proceed
  }
  ...
  async initFromUrl() {
    const params = new URLSearchParams(window.location.search);
    // const _pathname = window.location.pathname; // unused
    ...
    if (params.has(this.urlKeys.collectionSource)) {
      ...
      if (catalogSelect) {
        ...
        if (
          ...
          typeof this.catalogSelector.handleCatalogChange === 'function'
        ) {
  ...
  async restoreCatalogBrowserState(params, retryCount = 0) {
    if (!this.catalogBrowser || !this.viewModeToggle) {
      if (retryCount < 3) {
        ...
        console.error(
          '📍 Catalog browser components still not available after 3 retries, giving up'
        );
        return;
      }
    }
          ...
          if (catalog.endpoint) {
            ...
            if (params.has(this.urlKeys.collectionId)) {
              ...
              if (collection) {
                ...
                if (params.has(this.urlKeys.itemId)) {
                  // const _itemId = params.get(this.urlKeys.itemId); // unused
  ...
  async restoreSearchState(params) {
    ...
    if (params.has(this.urlKeys.search)) {
      ...
      if (searchInput) {
        searchInput.value = params.get(this.urlKeys.search);
      }
    }
    ...
    if (params.has(this.urlKeys.geometry)) {
      ...
      if (bboxInput) {
        ...
        if (this.mapManager?.displayBboxOnMap) {
          try {
            const coords = bboxValue.split(',').map(Number);
            if (coords.length === 4) {
              this.mapManager.displayBboxOnMap(
                coords,
                params.get(this.urlKeys.locationName) || 'URL Selection'
              );
            }
          } catch (error) {
    ...
    try {
      ...
      if (state.collection !== undefined) {
        const displayName = state.collection
          ? this.getCollectionDisplayName(state.collection, state.collectionSource)
          : 'EVERYTHING';
   ...
   */
  executeSearchAndWaitForResults() {
    return new Promise(resolve => {
      ...
      document.addEventListener('searchResultsLoaded', handleSearchResults, {
        once: true,
      });
      ...
      setTimeout(() => {
        if (this.searchPanel?.performSearch) {
   ...
   */
  updateUIForActiveItem(_activeItem) {
    setTimeout(() => {
  ...
  executeSearchAfterRestore() {
    try {
      ...
      if (
        ...
        typeof this.inlineDropdownManager.executeSearch === 'function'
      ) {
  ...
  updateURL() {
    ...
    if (collectionSelect?.value) {
      ...
      // Clean URL path detected - collection already in URL path
    }
    ...
    if (bboxInput?.value) {
      ...
      if (locationNameElement?.dataset.locationName) {
        params.set(this.urlKeys.locationName, locationNameElement.dataset.locationName);
      } else if (
        this.currentState?.locationName &&
        this.currentState.locationName !== 'THE WORLD'
      ) {
    ...
    try {
      ...
      if (Object.keys(restoredState).length > 0) {
        ...
        this.notificationService.showNotification(
          '[URL] Search parameters restored from URL',
          'info'
        );
      }
    } catch (error) {
  ...
  getCurrentCatalogId() {
    try {
      ...
      if (this.router?.getCurrentCatalogIdSync) {
  ...
  getCurrentCollectionId() {
    try {
      ...
      if (this.router?.getCurrentCollectionIdSync) {
