...
export class STACTileLayer extends TileLayer {
  ...
  renderSubLayers(props) {
    ...
    // const _tile = props._tile; // Unused parameter from props destructuring
...
export class STACCollectionVisualization {
  ...
  async addCollection(collection, options = {}) {
    try {
      ...
      if (!tileEndpoint) {
        ...
        return await this.addCollectionAsItems(collection, options);
      }
  ...
  findTileEndpoint(collection) {
    ...
    for (const link of links) {
      if (link.rel === 'tiles' || link.rel === 'wmts' || link.type?.includes('tiles')) {
        return link.href;
      }
    }
  ...
  async loadTile(tile, collection, options) {
    ...
    try {
      // Build tile URL
      let tileUrl = collection.tileEndpoint.replace('{z}', z).replace('{x}', x).replace('{y}', y);
   ...
   * @param {Object} options - Options
   */
  addCollectionAsItems(_collection, _options) {
...
export class EnhancedSTACRenderer {
  ...
  renderItem(item, options = {}) {
    ...
    try {
      ...
      if (this.isCOGAsset(asset)) {
        renderResult = this.renderCOGAsset(item, asset, options);
      } else if (this.isImageAsset(asset)) {
        renderResult = this.renderImageAsset(item, asset, options);
      } else {
   ...
   */
  isImageAsset(asset) {
    return asset.type?.startsWith('image/') && !this.isCOGAsset(asset);
  }
   ...
   * @param {Object} options - Options
   */
  renderCOGAsset(item, asset, options) {
   ...
   * @param {Object} options - Options
   */
  renderImageAsset(item, asset, options) {
   ...
   * @param {Object} options - Options
   */
  loadCOGTile(_tile, _asset, _options) {
