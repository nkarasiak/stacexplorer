...
export class CollectionDetailsModal {
    ...
    populateBasicInfo(collection) {
        ...
        const infoItems = [
            ...
            { label: 'Description', value: collection.description || 'N/A', icon: 'description', multiline: true },
            { label: 'License', value: collection.license || 'N/A', icon: 'gavel' },
            { label: 'Keywords', value: collection.keywords ? collection.keywords.join(', ') : 'N/A', icon: 'label' }
        ];
        
        basicInfo.innerHTML = infoItems.map(item => `
    ...
    populateExtentInfo(collection) {
        ...
        // Spatial extent
        if (collection.extent.spatial && collection.extent.spatial.bbox) {
        ...
        // Temporal extent
        if (collection.extent.temporal && collection.extent.temporal.interval) {
    ...
    populateProvidersInfo(collection) {
        ...
        const providersHTML = collection.providers.map(provider => `
            <div class="provider-item">
                ...
                ${provider.url ? `<a href="${provider.url}" target="_blank" class="provider-link">
    ...
    populateAssetsInfo(collection) {
        ...
        // Links
        if (hasLinks) {
            const relevantLinks = collection.links.filter(link => 
                !['self', 'parent', 'root'].includes(link.rel)
            );
    ...
    populatePropertiesInfo(collection) {
        ...
        const summariesHTML = Object.entries(collection.summaries).map(([key, summary]) => {
            let valueDisplay = '';
