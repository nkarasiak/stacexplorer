...
export class STACApiClient {
    ...
    try {
      ...
      if (normalizedUrl.endsWith('.json')) {
        ...
        if (this.endpoints.collections === '' && this.endpoints.search === '') {
          ...
          const collectionsLink = rootCatalog.links.find(
            link => link.rel === 'data' || link.rel === 'collections'
          );
          const searchLink = rootCatalog.links.find(link => link.rel === 'search');
  ...
  async fetchPlanetLabsCollections() {
    try {
      ...
      const childLinks = this.planetLabsCatalogData.links.filter(
        link => link.rel === 'child' || link.rel === 'catalog'
      );
  ...
  async fetchGEECollections() {
    try {
      ...
      // Find child links that point to collections or sub-catalogs
      const childLinks = this.geeCatalogData.links.filter(
        link => link.rel === 'child' || link.rel === 'catalog'
      );
  ...
  async fetchStaticCatalogCollections() {
    try {
      ...
      // Find child links that point to collections or sub-catalogs
      const childLinks = this.staticCatalogData.links.filter(
        link => link.rel === 'child' || link.rel === 'catalog'
      );
  ...
  isCurrentlyPlanetLabs() {
    ...
    const isPlanetLabs =
      selectedCatalog === 'planetlabs' ||
      // Also check if the current endpoints indicate Planet Labs
      this.endpoints.root?.includes('planet.com');
  ...
  async searchItems(params = {}) {
    try {
      ...
      if (features.length > 0) {
        // Sample cloud cover data (removed unused variable)
        ...
        if (params['eo:cloud_cover']) {
          ...
          if (exceedsFilter.length > 0) {
            console.warn(
              `🚨 WARNING: Found ${exceedsFilter.length} items exceeding cloud cover filter (>${filterValue}%):`,
          ...
          // No action needed when cloud cover filter check passes
        }
      }
   ...
   */
  presignPlanetaryComputerUrls(items) {
    if (!Array.isArray(items)) {
      return items;
    }

    return Promise.all(
      items.map(async (item, _itemIndex) => {
        if (!item.assets) {
          return item;
        }
        ...
        await Promise.all(
          Object.keys(item.assets).map(async assetKey => {
            const asset = item.assets[assetKey];

            const needsPresigning =
              asset.href &&
              (asset.href.includes('planetarycomputer') ||
                this.needsPlanetaryComputerPresigning(asset.href));
  ...
  async createPresignedSTACItem(stacItem) {
    ...
    for (const asset of Object.values(presignedItem.assets)) {
    ...
    await Promise.all(
      Object.keys(presignedItem.assets).map(async assetKey => {
        ...
        // Asset doesn't need presigning - continue processing
      })
    );
  ...
  async searchPlanetLabsItems(params = {}) {
    try {
      ...
      if (selectedCollection) {
        ...
        const childLinks = this.planetLabsCatalogData.links
          .filter(link => link.rel === 'child' || link.rel === 'catalog')
  ...
  async fetchItemsFromCatalog(catalogData, catalogUrl, limit = 10) {
    ...
    try {
      // Look for item links
      const itemLinks =
        catalogData.links?.filter(link => link.rel === 'item').slice(0, limit) || [];
      ...
      if (items.length === 0) {
        ...
        for (const childLink of childLinks) {
          try {
            ...
            if (response.ok) {
              const childData = await response.json();
              const childItems = await this.fetchItemsFromCatalog(
                childData,
                childUrl,
                Math.max(1, Math.floor(limit / childLinks.length))
              );
