...
import {
  // loadAllCollections, // unused
  getEnabledCollectionsFromSettings,
...
export class CollectionGridSelector {
  ...
  getThumbnailUrl(collection) {
    ...
    if (collection.links) {
      const previewLink = collection.links.find(
        link =>
          link.rel === 'preview' ||
  ...
  filterCollections() {
    ...
    // Apply search filter
    if (this.searchTerm) {
      filtered = filtered.filter(collection => collection.searchableText.includes(this.searchTerm));
    }

    // Apply source filter
    if (this.selectedSource !== 'all') {
      filtered = filtered.filter(collection => collection.source === this.selectedSource);
    }
  ...
  renderCollections() {
    ...
    grid.querySelectorAll('.collection-card').forEach((card, _index) => {
  ...
  selectCollection(collection) {
    ...
    const checkModalState = _step => {
      // const _modalOverlay = document.querySelector('.collection-browser-modal-overlay.open'); // Debug variable - not used
    };
  ...
  showCollectionDetails(collection) {
    ...
    document.dispatchEvent(
      new CustomEvent('showCollectionDetails', {
        detail: { collection },
      })
    );
  }
  ...
  disableSourceFilter() {
    ...
    this.elements.sourceFilter.querySelector('option[value="all"]').textContent =
      'Loading sources...';
    this.collectionsLoaded = false;
  }
  ...
  setSelectedCollection(collectionId, source = null) {
    const collection = this.collections.find(
      c => c.id === collectionId && (source ? c.source === source : true)
    );
