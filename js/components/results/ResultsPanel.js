...
export class ResultsPanel {
  ...
  constructor(apiClient, mapManager, notificationService) {
    ...
    window.testMapRestore = () => {
      if (this.savedMapView) {
        ...
        // Intentionally empty - no saved map view to restore
      }
    };
    ...
    window.debugHoverState = () => {
      // Intentionally empty - debug function placeholder for development
    };
   ...
   */
  createEnhancedItemContent(item, _formattedJson) {
            ...
            <div class="item-details-enhanced">
                ...
                <div class="simple-tabs">
                    <div class="tab-headers">
                        ...
                        ${
                          qualityInfo
                            ? `<button class="tab-header" data-target="quality-content">
                    ...
                    <div class="tab-contents">
                        ...
                        ${
                          qualityInfo
                            ? `<div class="tab-content-pane" id="quality-content">
                            ${qualityInfo}
                        </div>`
                            : ''
                        }
  ...
  getAssetIcon(key, asset) {
    ...
    if (keyLower.includes('thumbnail')) {
      return 'photo';
    }
    if (keyLower.includes('metadata')) {
      return 'description';
    }
  ...
  setupTabSwitching(content) {
    ...
    tabPanes.forEach((_pane, _index) => {
      // Intentionally empty - iteration placeholder for debugging tab panes
    });
  ...
  renderPage() {
    ...
    if (pageItems.length === 0) {
      datasetList.innerHTML =
        '<li class="dataset-item"><div class="dataset-content">No datasets found</div></li>';
      return;
    }
  ...
  createDatasetItem(item) {
    ...
    const isUsableUrl = url => {
      return (
        url && !url.startsWith('s3://') && (url.startsWith('http://') || url.startsWith('https://'))
      );
    };
    ...
    if (!hasThumbnail && item.assets) {
      ...
      if (!hasThumbnail) {
        // Intentionally empty - no thumbnail handling needed in this case
      }
    }
    ...
    if (!hasThumbnail) {
      ...
      } else if (item.collection?.includes('landsat')) {
    ...
    // Get the title
    const title = item.properties?.title ? item.properties.title : item.title || item.id;

    // Get the description (unused - keeping for potential future use)
    // const _description =
      ...
      thumbnail.onerror = () => {
        ...
        if (clickableCard) {
          ...
          clickableCard.addEventListener('click', e => {
            ...
            this.mapManager
              .displayItemOnMap(item, null, true)
              .then(() => {
              ...
              .catch(error => {
                ...
                document.getElementById('loading').style.display = 'none';
                this.notificationService.showNotification('Error displaying item on map', 'error');
              });
          });
        }
      };
  ...
  attachItemEventListeners(element, item) {
    ...
    // const _thumbnail = element.querySelector('.dataset-thumbnail'); // unused
    ...
    if (clickableCard) {
      clickableCard.addEventListener('click', e => {
        ...
        if (
          ...
          e.target.closest('.viz-btn') ||
          e.target.closest('.center-map-btn')
        ) {
          return;
        }
  ...
  showVisualizationPanel(item) {
    try {
      ...
      this.notificationService?.showNotification('Error opening visualization panel', 'error');
    }
  }
    ...
    try {
      ...
      if (isCurrentlyCentered) {
        ...
        if (bbox) {
          ...
          this.notificationService?.showNotification(
            'Unable to determine item location',
            'warning'
          );
          return;
        }
      ...
      this.notificationService?.showNotification('Error centering map on item', 'error');
    }
  }
  ...
  async copyItemInfo() {
    try {
      ...
      this.notificationService.showNotification('Failed to copy item information', 'error');
    }
  }
  ...
  async viewItemOnMap() {
    try {
      ...
      console.error('❌ Error displaying item on map:', error);
      this.notificationService.showNotification('Failed to display item on map', 'error');
    }
  }
  ...
  restoreMapView() {
    ...
    try {
      // const _currentCenter = map.getCenter(); // unused
      // const _currentZoom = map.getZoom(); // unused
      ...
      if (restored) {
        ...
        setTimeout(() => {
          try {
            // const _newCenter = map.getCenter(); // unused
            // const _newZoom = map.getZoom(); // unused
          } catch {
            // Intentionally empty - map operations may fail during cleanup, errors ignored
          }
        }, 700);
      }
  ...
  forceRestoreMapView() {
    ...
    if (this.savedMapView) {
      ...
      // Intentionally empty - no saved map view to force restore
    }
  }
  ...
  displayItemWithEvent(item, assetKey = null, source = 'user-click') {
    ...
    if (source === 'url' || source === 'navigation') {
      ...
      this.mapManager
        .displayItemOnMap(item, assetKey, true)
        .then(() => {
          // Show success notification
          this.notificationService.showNotification(
            `Viewing ${item.properties?.title || item.id} on map`,
