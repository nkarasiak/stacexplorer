...
class SimpleDeckGLIntegration {
  ...
  async addStacItemLayer(item, assetKey = null, options = {}) {
    ...
    try {
      ...
      let asset = null;
      // Asset key tracking removed as unused
      ...
      if (!asset) {
        console.warn(
          `No suitable preview asset found in item ${item.id}. Available assets:`,
      ...
      const layer = new window.deck.BitmapLayer({
        ...
        onError: error => {
          console.warn(
            '🔄 Deck.gl image loading failed (CORS/network issue), falling back to MapLibre:',
            asset.href
          );
      ...
      // Viewport preservation requested - no bounds fitting needed
  ...
  removeStacLayer() {
    if (this.currentLayer) {
      ...
      if (this.currentBlobUrl?.startsWith('blob:')) {
  ...
  async getImageAsBase64(imageUrl) {
    try {
      ...
      console.warn(
        '⚠️ Fetch failed, trying direct image loading with canvas conversion:',
        fetchError
      );
