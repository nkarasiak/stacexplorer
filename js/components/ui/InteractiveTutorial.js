...
export class InteractiveTutorial {
  constructor() {
    ...
    // Keeping for potential future use when tutorial is re-enabled
    /*
  ...
  initializeSteps() {
    return [
      ...
      },
      {
        ...
        position: 'right',
        buttons: [{ text: 'Next Step', action: 'next', primary: true }],
        action: () => {
      ...
      },
      {
        ...
        position: 'top',
        buttons: [{ text: 'Search Now!', action: 'next', primary: true }],
        action: () => {
      ...
      },
      {
        ...
        target: '#results-card',
        position: 'left',
        buttons: [{ text: 'Finish Tutorial', action: 'close', primary: true }],
      },
    ];
  }
      ...
      if (priorityCards.length > 0) {
        ...
        priorityCards.forEach((card, index) => {
          ...
          arrow.style.cssText = `
                    ...
                    `;
          arrow.textContent =
            index === 0 ? '⭐ Click this recommended collection!' : '⭐ Recommended';
  ...
  showCollectionButtonGuide() {
      ...
      mutations.forEach(mutation => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          ...
          if (target.classList.contains('open') && !modalOpenedAutomatically) {
            ...
            console.warn(
              '🚨 Tutorial: Modal opened automatically after ~0.5 seconds! This suggests there is code auto-clicking the SOURCE button.'
            );
    ...
    const buttonClickHandler = _event => {
  ...
  showDatePickerGuide() {
    ...
    [startDateInput, endDateInput].forEach((input, _index) => {
  ...
  listenForModalClose() {
    ...
    const collectionSelectionHandler = _event => {
      this.cleanupCollectionButtonGuide();
      this.cleanupCollectionGuides();
  ...
  listenForLocationSearchClose() {
    ...
    const locationSelectionHandler = _event => {
    ...
    const checkLocationSearchClosed = () => {
      ...
      const isLocationActive =
        (locationInput && document.activeElement === locationInput) ||
        (locationDropdown &&
  ...
  listenForDatePickerClose() {
    ...
    const dateChangeHandler = _event => {
    ...
    const checkDatePickerClosed = () => {
      ...
      const isDateActive =
        (startDateInput && document.activeElement === startDateInput) ||
  ...
  updateButtons(step) {
    ...
    const buttonsHtml = step.buttons
      .map(button => {
