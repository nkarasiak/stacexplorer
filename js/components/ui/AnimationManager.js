...
export class AnimationManager extends BaseUIComponent {
    ...
    trackAnimationPerformance(entry) {
        if (this.options.adaptiveQuality && entry.duration > 16.67) { // Slower than 60fps
     ...
     */
    async createCSSAnimation(element, keyframes, config) {
     ...
     */
    async createJSAnimation(element, keyframes, config) {
        const id = this.generateAnimationId();
        let startTime = null;
        let isRunning = true;
        let isPaused = false;
     ...
     * @returns {Promise} Animation promise
     */
    async spring(element, fromValues, toValues, options = {}) {
            ...
            const animate = () => {
                ...
                for (const property in toValues) {
                    ...
                    if (Math.abs(current - target) > 0.01) {
                        ...
                        this.setElementProperty(element, property, spring.value + (toValues[property].toString().match(/[a-z%]+$/i)?.[0] || ''));
                    }
                }
     ...
     */
    async stagger(elements, keyframes, options = {}) {
        const config = {
            delay: 100,
        ...
        try {
            for (let i = 0; i < steps.length; i++) {
                ...
                if (step.parallel) {
                    ...
                    await Promise.all(step.animations.map(anim => 
                        this.animate(anim.element, anim.keyframes, anim.options)
                    ));
                } else {
     ...
     */
    async handleReducedMotion(element, keyframes, options) {
