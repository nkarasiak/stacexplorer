...
import { geojsonToBbox, isGeoJSON, isWKT, parseGeoJSON, wktToGeoJSON } from './GeometryUtils.js';
...
export class GeometrySync {
  ...
  setupEventListeners() {
    ...
    if (bboxInput) {
      ...
      bboxInput.addEventListener('paste', _e => {
        setTimeout(() => {
  ...
  handleBboxCoordinates(bboxText) {
    try {
      ...
      if (coords.length === 4 && coords.every(coord => !Number.isNaN(coord))) {
        ...
        if (
          ...
          if (this.mapManager) {
            ...
            if (typeof this.mapManager.fitToBounds === 'function') {
              ...
              this.mapManager.mapLayers &&
              typeof this.mapManager.mapLayers.fitMapToBbox === 'function'
            ) {
          ...
          if (this.notificationService) {
            this.notificationService.showNotification(
              '✅ Coordinates applied to AI Search',
              'success'
            );
          }
        } else {
  ...
  parseAndSyncGeometry(geometryText, source = 'unknown') {
    try {
      ...
      if (
        ...
        typeof this.inlineDropdownManager.handleLocationSelection === 'function'
      ) {
        try {
      ...
      if (this.notificationService) {
        this.notificationService.showNotification(
          `✅ ${geometryType} parsed and added to AI Search!`,
          'success'
        );
      }
      ...
      if (this.notificationService) {
        this.notificationService.showNotification(
          `❌ Invalid ${error.message.includes('WKT') ? 'WKT' : error.message.includes('GeoJSON') ? 'GeoJSON' : 'geometry'} format`,
          'error'
        );
      }
  ...
  handleBboxDrawn(bbox) {
    if (Array.isArray(bbox) && bbox.length === 4) {
      ...
      if (this.notificationService) {
        this.notificationService.showNotification(
          '✅ Map selection added to search form',
          'success'
        );
      }
    }
  ...
  updateMainBboxInput(bbox) {
    ...
    if (bboxInput) {
      ...
      console.warn('✅ Updated bbox input:', bboxString);
