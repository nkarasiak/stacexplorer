...
export class CacheManager extends BaseUIComponent {
  ...
  setInMemory(key, entry, _options = {}) {
  ...
  evictFromMemory() {
    if (this.state.memoryCache.size === 0) {
      return false;
    }
  ...
  evictFromDisk() {
    ...
    return new Promise(resolve => {
      try {
        ...
        request.onerror = () => {
          console.warn('Disk cache eviction error:', request.error);
          resolve();
        };
      } catch (error) {
  ...
  attachPerformanceObserver() {
    if (!this.options.enableMetrics || !('PerformanceObserver' in window)) {
      return;
    }
  ...
  cleanupDiskCache() {
    return new Promise(resolve => {
      try {
        ...
        request.onerror = () => {
          console.warn('Disk cache cleanup error:', request.error);
          resolve();
        };
      } catch (error) {
  ...
  clearDiskPattern(pattern) {
    ...
    return new Promise(resolve => {
      try {
        ...
        request.onerror = () => {
          console.warn('Pattern-based disk cache clear error:', request.error);
          resolve();
        };
      } catch (error) {
  ...
  destroy() {
    ...
    if (this.state.serviceWorkerRegistration) {
      this.state.serviceWorkerRegistration.removeEventListener(
        'message',
