...
export class LoadingStateManager extends BaseUIComponent {
    ...
    getDefaultSkeletonTemplate(config) {
        ...
        if (container?.classList.contains('dataset-list') || 
            container?.dataset.type === 'stac-items') {
            return this.getSTACItemsSkeletonTemplate();
        }
        
        if (container?.classList.contains('collection-list') ||
     ...
     */
    async showLoadingElement(loadingState) {
     ...
     * @param {Object} loadingState - Loading state object
     */
    async hideLoadingElement(loadingState) {
        const { element } = loadingState;
        
        if (!element || !loadingState.isVisible) return;
    ...
    async hideAllLoading() {
        const promises = Array.from(this.loadingStates.keys())
            .map(key => this.hideLoading(key));
