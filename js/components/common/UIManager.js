...
export class UIManager {
  ...
  initializeCollapsibleCards() {
    ...
    cardIds.forEach(cardId => {
      ...
      // Card element reference not needed here
  ...
  toggleToolsPanel() {
    ...
    // Toggle icon reference not needed here
  ...
  updateGPUStatus(context = 'modal') {
    ...
    try {
      ...
      } else if (window.app?.mapManager) {
        mapManager = window.app.mapManager;
      }

      if (mapManager?.deckGLIntegration) {
  ...
  handleCatalogToggle(catalogKey, isEnabled) {
    ...
    document.dispatchEvent(
      new CustomEvent('catalogToggled', {
        detail: {
    ...
    if (customCatalogInput) {
      ...
      if (addCustomCatalogBtn) {
        addCustomCatalogBtn.addEventListener('click', e => {
          ...
          // No action needed when URL input is empty
        });
      } else {
  ...
  async validateAndAddCustomCatalog(url) {
    try {
      ...
      document.dispatchEvent(
        new CustomEvent('customCatalogAdded', {
  ...
  showCustomCatalogStatus(message, type = 'info') {
    ...
    if (!statusEl && type !== 'hidden') {
      ...
      const customCatalogSection = document
        .querySelector('#custom-catalog-url')
        .closest('.setting-item');
      if (customCatalogSection) {
        statusEl = document.createElement('div');
        statusEl.id = 'custom-catalog-status';
        statusEl.style.cssText =
          'margin-top: 8px; padding: 6px 12px; border-radius: 4px; font-size: 13px; display: flex; align-items: center; gap: 6px;';
