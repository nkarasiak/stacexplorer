...
export class SearchForm {
      ...
      bboxInput.addEventListener('change', event => {
        if (this.mapManager && event.target.value) {
          try {
            ...
            if (bboxValues.length === 4 && !bboxValues.some(Number.isNaN)) {
  ...
  isNonOpticalCollection(collections) {
    ...
    return nonOpticalKeywords.some(keyword => collectionIds.includes(keyword));
  }
  ...
  getSearchParams() {
    ...
    try {
      ...
      if (searchText) {
        params.query = {
          or: [{ contains: ['title', searchText] }, { contains: ['description', searchText] }],
        };
      }
      ...
      if (window.stacExplorer?.filterManager) {
        ...
        if (cloudCoverEnabled?.checked && cloudCoverInput && !cloudCoverInput.disabled) {
          ...
          if (!Number.isNaN(cloudCoverValue)) {
            ...
            if (!isNonOpticalData) {
              ...
              // Intentionally empty - non-optical data doesn't use cloud cover filter
            }
          }
        }
      }
