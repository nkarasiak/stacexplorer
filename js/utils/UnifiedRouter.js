...
export class UnifiedRouter {
  ...
  createRoutePatterns() {
    ...
    return {
      ...
      legacyBrowserVerbose:
        /^\/browser\/catalog\/([^/]+)(?:\/collection\/([^/]+))?(?:\/item\/([^/]+))?\/?$/,
    };
  }
  ...
  setupEventListeners() {
    ...
    document.addEventListener('viewModeChanged', event => {
      ...
      // Route processing in progress - ignore view mode change
    });
    ...
    document.addEventListener('catalogBrowserStateChanged', event => {
      ...
      // Route processing in progress or not in browser mode - ignore state change
    });
    ...
    document.addEventListener('searchParameterChanged', event => {
      ...
      // Route processing in progress or not in view mode - ignore search parameter change
    });
    ...
    document.addEventListener('itemActivated', event => {
      ...
      if (this.currentMode === 'view' && !isInBrowserMode) {
        ...
        // No action needed when not updating item path
      }
      // Route processing in progress - ignore item selection
    });
    ...
    document.addEventListener('collectionSelected', event => {
      ...
      // Route processing in progress or not in view mode - ignore collection selection
    });
    ...
    document.addEventListener('geometrySelected', event => {
      ...
      // Route processing in progress or not in view mode - ignore geometry selection
    });
  }
    ...
    try {
      ...
      switch (route.name) {
        ...
        case 'viewCatalogCollectionItem':
          await this.handleViewCatalogCollectionItemRoute(
            route.matches[0],
            route.matches[1],
            route.matches[2]
          );
  ...
  async handleBrowserItemRoute(catalogId, collectionId, itemId) {
    ...
    debugLog(
      '📍 Handling browser item route - showing item page: ' +
        catalogId +
        ' ' +
        collectionId +
        ' ' +
    ...
    try {
      ...
      // Set up API client
      const apiClient =
        this.stateManager.apiClient || window.stacExplorer?.catalogBrowser?.apiClient;
      if (apiClient) {
      ...
      if (item) {
        ...
        debugLog(
          '📍 Item not found in collection. Available items: ' +
            itemsArray
              .map(i => i.id)
              .slice(0, 10)
              .join(', ')
  ...
  updateViewPath(stateChange) {
    ...
    // const _urlKeys = this.stateManager.urlKeys; // unused
    ...
    if (stateChange.type === 'item' && stateChange.itemId) {
      ...
      if (catalogId && collectionId) {
        ...
        newPath = this.createPath(
          `/viewer/${encodeURIComponent(catalogId)}/${encodeURIComponent(collectionId)}/${encodeURIComponent(stateChange.itemId)}`
        );
      } else {
      ...
      if (catalogId) {
        ...
        newPath = this.createPath(
          `/viewer/${encodeURIComponent(catalogId)}/${encodeURIComponent(stateChange.collection)}`
        );
      } else {
        // Fallback to legacy collection-only format
        newPath = this.createPath(
          `/viewer/collection/${encodeURIComponent(stateChange.collection)}`
        );
      }
    } else if (
      stateChange.search ||
      (stateChange.locationBbox && !this.isCurrentlyInCatalogCollectionView()) ||
      (stateChange.dateStart && !this.isCurrentlyInCatalogCollectionView())
    ) {
    ...
    // Legacy upgrade not needed - URL already in correct format
  }
  ...
  updateBrowserPath(state) {
    ...
    // Path unchanged - no history update needed
  }
    ...
    Object.entries(stateChange).forEach(([key, value]) => {
      switch (key) {
        ...
        case 'collectionSource':
          if (value) {
            params.set(urlKeys.collectionSource, value);
          }
          break;
        ...
        case 'dateStart':
          if (value) {
            params.set(urlKeys.dateStart, value);
          }
          break;

        case 'dateEnd':
          if (value) {
            params.set(urlKeys.dateEnd, value);
          }
          break;
  ...
  async getAvailableCatalogs() {
    ...
    if (
      this.stateManager.catalogBrowser &&
      typeof this.stateManager.catalogBrowser.getAvailableCatalogs === 'function'
    ) {
      try {
  ...
  getCurrentCatalogIdSync() {
    try {
      ...
      if (this.stateManager.apiClient?.endpoints) {
        const currentEndpoint = this.stateManager.apiClient.endpoints.root;

        // Check against known catalog endpoints from config
        if (window.stacExplorer?.config?.stacEndpoints) {
          for (const [configKey, endpoint] of Object.entries(
            window.stacExplorer.config.stacEndpoints
          )) {
  ...
  getCurrentCollectionIdSync() {
    try {
      ...
      if (this.stateManager.collectionManager?.getSelectedCollection) {
  ...
  async getCurrentCatalogId() {
    try {
      ...
      if (this.stateManager.apiClient?.endpoints) {
        ...
        const availableCatalogs = await this.getAvailableCatalogs();
        const matchingCatalog = availableCatalogs.find(
          catalog => catalog.endpoint && catalog.endpoint.root === currentEndpoint
        );
