...
export class RasterVisualizationManager {
   ...
   */
  addSTACItemLayer(stacItem, preset = 'true-color', options = {}) {
    try {
      ...
      const { sourceId } = this.createTiTilerLayer(
        null,
        selectedPreset,
        layerId,
        options,
      ...
      this.dispatchLayerEvent('layerAdded', {
        layerId,
        item: stacItem,
        preset,
        presetConfig: selectedPreset,
   ...
   */
  createTiTilerLayer(_stacItemUrl, presetConfig, layerId, options = {}, stacItem = null) {
    ...
    if ((presetConfig.assets && presetConfig.assets.length > 1) || presetConfig.expression) {
      ...
      if (selfLink) {
        ...
        // Intentionally empty - fallback to stacItemUrl if no self link found
      }
    } else {
      // Intentionally empty - stacItemUrl already provided
    }
    ...
    // const _testTileUrl = urlTemplate
    //   .replace('{z}', '10')
    //   .replace('{x}', '500')
    //   .replace('{y}', '300'); // Debug variable - not currently used
    ...
    setTimeout(() => {
      ...
      if (layerExists) {
        // const _opacity = this.mapManager.getMap().getPaintProperty(layerId, 'raster-opacity'); // Debug variables
        // const _visibility = this.mapManager.getMap().getLayoutProperty(layerId, 'visibility');
      }
      if (sourceExists) {
        // const _source = this.mapManager.getMap().getSource(sourceId); // Debug variable
      }
    }, 100);
   ...
   */
  changeVisualization(layerId, newPreset) {
  ...
  zoomToItem(stacItem, padding = 0.1) {
    if (stacItem.bbox && stacItem.bbox.length >= 4) {
      ...
      // const _bounds = [ // Debug variable - not used
      //   [west, south],
      //   [east, north],
      // ];
  ...
  fadeOutMapLibreLayer(layerId, callback) {
    ...
    // Fade out animation (currently disabled)
    // const steps = 10;
  ...
  dispatchLayerEvent(eventType, detail = {}) {
    document.dispatchEvent(
      new CustomEvent(`rasterVisualization:${eventType}`, {
        detail: {
          ...detail,
          manager: this,
          timestamp: Date.now(),
  ...
  getStats() {
    return {
      ...
      collections: [
        ...new Set(Array.from(this.currentLayers.values()).map(info => info.item.collection)),
      ],
    };
  }
