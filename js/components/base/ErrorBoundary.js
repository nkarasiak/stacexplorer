...
export class ErrorBoundary extends BaseUIComponent {
   ...
   */
  isDuplicateError(error, _errorInfo) {
    ...
    return timeDiff < 1000 && lastError.error.message === error.message;
  }
   ...
   */
  showError(errorInfo) {
    if (!this.options.showErrorOverlay) {
      return;
    }
  ...
  handleToggleDetails() {
    ...
    if (toggle && content) {
      ...
      toggle.querySelector('.material-icons').textContent = isExpanded
        ? 'expand_less'
        : 'expand_more';
...
export class ErrorBoundaryManager {
  ...
  unregister(boundaryOrId) {
    const id = typeof boundaryOrId === 'string' ? boundaryOrId : boundaryOrId.componentId;
