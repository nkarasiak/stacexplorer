...
export class BandCombinationEngine {
  ...
  findAssetByBandCharacteristics(genericName, stacItem) {
    ...
    if (stacItem.assets?.[genericName]) {
      return genericName;
    }
  ...
  checkUrlAccessibility(url) {
    try {
      ...
      const isValidUrl =
        url.startsWith('https://') || url.startsWith('http://') || url.startsWith('s3://');
  ...
  async checkAssetsAccessibility(stacItem, assetNames) {
    ...
    if (results.inaccessible.length > 0) {
      // Intentionally empty - inaccessible assets logged elsewhere
    }
  ...
  getPresetByKey(presetKey) {
    for (const [, presets] of this.presets) {
  ...
  async canUseTiTiler(stacItem, presetKey = null) {
    ...
    if (presetKey) {
      ...
      const mappedAssets = this.mapGenericToActualAssets
        ? this.mapGenericToActualAssets(preset.assets, stacItem)
        : preset.assets;
   ...
   */
  async buildTileUrlWithAccessibilityCheck(
    stacItemUrl,
    preset,
    z,
    x,
  ...
  buildTileUrl(stacItemUrl, preset, z, x, y, stacItem = null, scaleOptions = {}) {
    ...
    if (
      (preset.assets && preset.assets.length === 1 && !preset.expression && stacItem) ||
      (stacItemUrl === null && stacItem && !preset.expression)
    ) {
      const assetName = preset.assets[0];
      const actualAssetName = this.mapGenericToActualAssets
        ? this.mapGenericToActualAssets([assetName], stacItem)[0]
        : assetName;

      if (stacItem.assets?.[actualAssetName]) {
        ...
        const rescaleValue =
          scaleOptions.minScale !== undefined && scaleOptions.maxScale !== undefined
            ? `${scaleOptions.minScale},${scaleOptions.maxScale}`
            : preset.rescale;
        if (rescaleValue) {
          params.set('rescale', rescaleValue);
        }
        if (preset.colormap_name) {
          params.set('colormap_name', preset.colormap_name);
        }
        if (preset.resampling) {
          params.set('resampling_method', preset.resampling);
        }
    ...
    if (preset.assets && preset.assets.length > 1 && !preset.expression && stacItem) {
      ...
      if (stacItem.assets) {
        for (const [, asset] of Object.entries(stacItem.assets)) {
    ...
    if (stacItemUrl === null && !preset.expression) {
      console.error(
        `❌ [TITILER] Cannot use STAC endpoint with null stacItemUrl and no valid single asset found`
      );
      console.error(`❌ [TITILER] Preset:`, preset);
      console.error(`❌ [TITILER] STAC item assets:`, Object.keys(stacItem?.assets || {}));
      throw new Error(
        `No valid asset found for visualization preset '${preset.name}'. Available assets: ${Object.keys(stacItem?.assets || {}).join(', ')}`
      );
    }
   ...
   */
  buildPreviewUrl(stacItemUrl, preset, bbox, width = 256, height = 256, stacItem = null) {
